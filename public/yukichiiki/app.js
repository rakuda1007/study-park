(function () {
  ...
  const store = window.YukichiikiStorage;
  const QUESTIONS = window.YUKICHIIKI_QUESTIONS || [];
  const TOTAL = window.YUKICHIIKI_TOTAL || QUESTIONS.length;
  const CHARS = window.YUKICHIIKI_CHARACTERS || [];
  const SQUAD_IDS = window.YUKICHIIKI_SQUAD_IDS || ["orange", "dog", "cat", "tofu"];
