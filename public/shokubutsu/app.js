(function () {
  ...
  const store = window.ShokubutsuStorage;
  const QUESTIONS = window.SHOKUBUTSU_QUESTIONS || [];
  const TOTAL = window.SHOKUBUTSU_TOTAL || QUESTIONS.length;
  ...
  function masteredMessage(n) {
    ...
    if (count >= TOTAL) return "植物の育ち方マスター！ ぜんぶおぼえたね！";
